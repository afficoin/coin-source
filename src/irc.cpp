...
// Copyright (c) 2011-2012 AffiCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AffiCoinTEST0\r");
            Send(hSocket, "WHO #AffiCoinTEST0\r");
        } else {
            // randomly join #AffiCoin00-#AffiCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #AffiCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #AffiCoin%02d\r", channel_number).c_str());
        }
