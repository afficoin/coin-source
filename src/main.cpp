...
uint256 hashGenesisBlock("0x3072a556a70f02a1ea2c3da25fe2aeeaaf9ad823d93100bcf98a2eb244c4f071");
...
const string strMessageMagic = "AffiCoin Signed Message:\n";
...
{
    // AffiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (pblock->nTime - pindexLast->nTime > nTargetSpacing*2000)
        return bnProofOfWorkLimit.GetCompact();
...
{
    // AffiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "affi genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1509050764;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3329079;
...
unsigned char pchMessageStart[4] = { 0xd7, 0xca, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AffiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AffiCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AffiCoinMiner(CWallet *pwallet)
{
    printf("AffiCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AffiCoin-miner");
    ...
    try { loop {
        ...
        printf("Running AffiCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AffiCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AffiCoinMiner, pwallet));
}
